...
/* ------------------------------------------------------------------ */
/* Normalize Advanced Dasha → local DashaReport                       */
/* ------------------------------------------------------------------ */

function normalizeDashaLike(anyDasha: any): DashaReport | null {
  if (!anyDasha) return null;

  const mahaSrc =
    anyDasha.mahaDashaTimeline ??
    anyDasha.mahaTimeline ??
    anyDasha.mahadasha ??
    anyDasha.maha;

  const yearlySrc =
    anyDasha.yearlyDashaTimeline ??
    anyDasha.yearlyTimeline ??
    anyDasha.annualDashaTimeline ??
    anyDasha.annual ??
    anyDasha.yearly;

  if (!yearlySrc || !Array.isArray(yearlySrc)) return null; // yearly is mandatory for Forecast

  const toDate = (v: any) => (v instanceof Date ? v : new Date(v));

  const mapSpans = (arr: any[]): DashaSpan[] =>
    (arr || []).map((x) => ({
      dashaNumber: Number(x?.dashaNumber ?? x?.dasha ?? x?.num ?? x?.value ?? 0),
      startDate: toDate(x?.startDate ?? x?.start ?? x?.from),
      endDate: toDate(x?.endDate ?? x?.end ?? x?.to),
      year: x?.year ?? (x?.startDate ? toDate(x.startDate).getFullYear() : undefined),
      label: x?.label,
    }));

  return {
    mahaDashaTimeline: Array.isArray(mahaSrc) ? mapSpans(mahaSrc) : [],
    yearlyDashaTimeline: mapSpans(yearlySrc),
  };
}
...
const ForecastTab: React.FC<Props> = ({ report, dashaReport, gender, dobStr, language = "en" }) => {
  // Prefer Advanced Dasha context timelines to keep Forecast in sync
  const { dashaReport: ctxDashaRaw, advancedDasha } = useReport() || {};
  const ctxDashaReport =
    normalizeDashaLike(ctxDashaRaw) ||
    normalizeDashaLike(advancedDasha) ||
    null;
  ...
  const safeDashaReport = useMemo(() => {
    // 1) Advanced Dasha (context)
    if (ctxDashaReport) return ctxDashaReport;
    // 2) Prop (already DashaReport shape)
    if (dashaReport) return dashaReport;
    // 3) Fallback if only dobStr exists
    if (dobStr) return buildDashaReportFallback(dobStr);
    return null;
  }, [ctxDashaReport, dashaReport, dobStr]);
